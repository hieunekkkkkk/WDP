...
class BusinessRevenueController {
  ...
  async getRevenueById(req, res) {
    try {
      const { id } = req.params;
      const revenue = await BusinessRevenueService.getRevenueById(id);
      res.status(200).json(revenue);
    } catch (err) {
      res.status(404).json({ error: 'Revenue not found', details: err.message });
    }
  }
  ...
  async updateRevenue(req, res) {
    try {
      const { id } = req.params;
      const revenue = await BusinessRevenueService.updateRevenue(id, req.body);
      res.status(200).json(revenue);
    } catch (err) {
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
