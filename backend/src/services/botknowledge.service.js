...
class BotKnowledgeService {
  ...
  async indexBotKnowledge(botId) {
    try {
      ...
      //  Lọc bỏ các document rỗng hoặc quá ngắn
      const validDocs = splitDocs.filter(
        (doc) => doc.pageContent && doc.pageContent.trim().length > 10
      );

      if (validDocs.length === 0) {
        console.warn(` No knowledge to index for bot ${botId}`);
        return;
      }

      // Log thống kê
      console.log(
        ` Chuẩn bị index ${validDocs.length}/${splitDocs.length} documents cho bot ${botId}`
      );
