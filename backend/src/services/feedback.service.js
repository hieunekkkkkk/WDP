...
class FeedbackService {
  async createFeedback(feedbackData) {
    if (feedbackData.business_id) {
      feedbackData.business_id = new mongoose.Types.ObjectId(
        feedbackData.business_id
      );
    }
    if (feedbackData.product_id) {
      feedbackData.product_id = new mongoose.Types.ObjectId(
        feedbackData.product_id
      );
    }
    try {
  ...
  async findFeedbackByBusinessId(businessId) {
    try {
      ...
      throw new Error(
        "Error retrieving feedback by business ID: " + error.message
      );
    }
  }
  ...
  async findFeedbackByProductId(productId) {
    try {
      ...
      throw new Error(
        "Error retrieving feedback by product ID: " + error.message
      );
    }
  }
